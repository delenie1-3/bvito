...
class RegisterUserForm(forms.ModelForm):#класс формы нового пользователя
    email = forms.EmailField(required=True, label='Адрес электронной почты')
    ...
    def clean_password1(self):#валидация пароля
        password1 = self.cleaned_data['password1']
        if password1:
            password_validation.validate_password(password1)
        return password1

    def clean(self):#проверка введённых паролей
        super().clean()
        password1 = self.cleaned_data['password1']
        password2 = self.cleaned_data['password2']
        if password1 and password2 and password1 != password2:
            errors = {'password2':ValidationError(
                'Введённые пароли не совпадпют', code='password_mismatch')}
            raise ValidationError(errors)

    def save(self, commit=True):# сохранение пароля
    ...
    class Meta:# Обозначение полей формы
        model = AdvUser
